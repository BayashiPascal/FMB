...
Overall, FMB is faster than SAT, at least if the percentage of intersecting Frames is low, for all cases but the 2D dynamic one. In practice, for example in applications where the Frames represents real world objects supposedly normally not in intersection, would be a better choice than SAT.\\
...
For one given pair in intersection, all the conditions must be checked before the algorithms give their answer. The algorithm with the fewest number of conditions is then the fastest, and the results shows that this is generally SAT (the number of conditions in FMB varies for each pair so we can't be more specific). This is shown in the result by the low variability of the ratio timeFMB/timeSAT for intersecting pairs.\\

For one given pair not in intersection, the algorithms reply as soon as one condition is verified. This may be the first one, as it may be the last one depending on the pair of Frames. Then, the variability of the ratio timeFMB/timeSAT varies widely, from 50 times faster to 29 times slower, but the resuts shows that in average the advantage goes to the FMB algorithm.\\
